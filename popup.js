(function() {
	...
	function createDiv(subscription) {
		...
		var time = liveNotification.formatTime(new Date(subscription.startTime)) + " <=> "
		if (subscription.status === "2") {
			time = time + "现在"
		} else {
			time = time + liveNotification.formatTime(new Date(subscription.endTime))
		}
	...
	document.getElementById("go-to-options").addEventListener("click", liveNotification.openOptionsPage)
